...
def run():
    ...
    def apply_model_to_embeddings_in_chunks(embeddings_path, model, output_path, chunk_size=200):
        ...
        output_zarr = zarr.open(output_path, shape=output_shape, dtype=int, mode='w')

        # Iterate through embeddings in chunks
        for z in range(0, zarr_embeddings.shape[0], chunk_size):
            for y in range(0, zarr_embeddings.shape[1], chunk_size):
                for x in range(0, zarr_embeddings.shape[2], chunk_size):
                    # Define slice for the current chunk with correct dimensions
                    input_slice = (slice(z, min(z + chunk_size, zarr_embeddings.shape[0])),
                                   slice(y, min(y + chunk_size, zarr_embeddings.shape[1])),
                                   slice(x, min(x + chunk_size, zarr_embeddings.shape[2])))
                    ...
                    # Calculate correct output slice considering the transposed dimensions
                    output_slice = (slice(x, min(x + chunk_size, output_shape[0])),
                                    slice(y, min(y + chunk_size, output_shape[1])),
                                    slice(z, min(z + chunk_size, output_shape[2])))

                    # Adjust output slice if necessary to fit the output shape
                    adjusted_output_slice = tuple(slice(s.start, min(s.stop, output_shape[dim])) for dim, s in enumerate(output_slice))
